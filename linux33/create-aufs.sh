...
rm -rf ../tmp/linux-3.3
mkdir -p ../tmp/linux-3.3
cp -a fs ../tmp/linux-3.3
cp -a include ../tmp/linux-3.3
cp -a Documentation ../tmp/linux-3.3
echo "test"
rm ../tmp/linux-3.3/include/linux/Kbuild
cd ../tmp
diff -Naur null linux-3.3  | filterdiff | \
sed -e 's|null\(/include/linux/Kbuild\)|linux-3.3-old\1|;s|^--- null.*|--- /dev/null|;\|linux-3.3/include/linux/Kbuild|,${\|@@ -0,0 +1 @@|,$d}' \
| bzip2 > aufs$(sed -ne 's|#define.*AUFS_VERSION.*"\(.*\)"|\1|p'  linux-3.3/include/linux/aufs_type.h).patch.bz2
mv *.bz2 $OLDPWD
cd $OLDPWD
mv *patch* ..
cd ..
